...
export class KeyManager implements KeyManagerPlugin {
  ...
  async checkAliasExists({ keyAlias }: { keyAlias: string }): Promise<{ aliasExists: boolean }> {
    ...
    return {
      aliasExists: (
        await Promise.all([this.keyStore.getCryptoKey(keyAlias), this.keyStore.getCryptoKeyPair(keyAlias)])
      ).some(Boolean),
    };
  }
  ...
  async generateRecoverableAgreementKeyPair(
    keyWrapParams: KeyWrapParams,
  ): Promise<{ recoverableKeyPair: RecoverableKeyPair }> {
    const keyPair = await webCrypto.subtle.generateKey(
      {
  ...
  private addSaltToUnwrapParams(
    unwrapParams: KeyUnwrapParams,
    recoverableKey: RecoverableKey,
  ): KeyUnwrapParamsWithSalt {
  ...
  private fillGeneratedSaltToUnwrapParams(unwrapParams: KeyUnwrapParams): KeyUnwrapParamsWithSalt {
    if (isPasswordParams(unwrapParams)) {
      ...
      return {
        ...unwrapParams,
        salt: salt ?? base64Encode(webCrypto.getRandomValues(new Uint8Array(128)).buffer),
      };
    }
    return unwrapParams;
  }

  private async unwrapSignaturePrivateKey(
    recoverableKey: RecoverableKey,
    unwrapParams: KeyUnwrapParams,
    extractable: boolean,
  ): Promise<CryptoKey> {
    const wrappingKey = await this.resolveWrappingKey(this.addSaltToUnwrapParams(unwrapParams, recoverableKey));
  ...
  private async unwrapAgreementPrivateKey(
    recoverableKey: RecoverableKey,
    unwrapParams: KeyUnwrapParams,
    extractable: boolean,
  ): Promise<CryptoKey> {
  ...
  private async wrapKeyWithKey(keyToWrap: CryptoKey, wrappingKey: CryptoKey): Promise<EncryptedMessage> {
    ...
    const key = await webCrypto.subtle.wrapKey(
      keyToWrap.algorithm.name == 'AES-GCM' ? 'raw' : 'pkcs8',
      keyToWrap,
      wrappingKey,
      {
        name: 'AES-GCM',
  ...
  private async resolveAgreementKeys(privateKeyAlias: string, publicKeyAlias: string): Promise<CryptoKeyPair> {
    ...
    const publicKey: CryptoKey | null =
      privateKeyAlias === publicKeyAlias ? keyPair.publicKey : await this.keyStore.getCryptoKey(publicKeyAlias);
  ...
  private async keyPairToRecoverableKeyPair(
    keyPair: CryptoKeyPair,
    wrapParams: KeyWrapParams,
  ): Promise<RecoverableKeyPair> {
  ...
  private async deriveKeyFromPassword(password: string, salt: ArrayBuffer) {
    ...
    const passwordKey = await webCrypto.subtle.importKey('raw', enc.encode(password), { name: 'PBKDF2' }, false, [
      'deriveBits',
      'deriveKey',
    ]);
